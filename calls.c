     ...
     int is_const;
{
  ...
  if (stack_size != 0)
	...
	adjust_stack (stack_size_rtx, GSP1);
    }
#endif
}
     ...
     int ignore;
{
  ...
  if (pending_stack_adjust >= 32 * GSP8
  ...
  for (p = actparms, argpos = 0; p; p = TREE_CHAIN (p), i += inc, argpos++)
	...
	args[i].size.constant -= ((args[i].partial * GSP8 * UNITS_PER_WORD)
  ...
  if (args_size.var != 0)
	...
	argblock = push_block (GEN_INT (needed / GSP8), 0, 0);
  ...
  if (argblock == 0)
    anti_adjust_stack (GEN_INT (args_size.constant
				- original_args_size.constant), GSP1);
#endif
#endif
  ...
  if (must_preallocate == 0 && reg_parm_stack_space > 0)
    anti_adjust_stack (GEN_INT (reg_parm_stack_space), GSP1);
#endif
     ...
     int reg_parm_stack_space;
{
  ...
  if (arg->value == arg->stack)
      ...
      else
	{
	  ...
	  excess = (arg->size.constant / GSP8 - int_size_in_bytes (TREE_TYPE (pval))

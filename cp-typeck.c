...
#ifdef TMS34010
	  idx = build_binary_op(MULT_EXPR, idx, size_int(GSP8), 0);
#endif
...
#ifdef TMS34010
	ind = build_binary_op(MULT_EXPR, ind, size_int(GSP8), 0);
#endif
...
#else
      { "obxX",		0,	T_UI,	T_UI,	T_UL,	NULL,	"-wp0#" },
#endif
...
typedef struct
{
  char *function_ident;		/* identifier such as "printf" */
...
void
_record_format_info (function_ident, is_scan, format_num, first_arg_num)
      char *function_ident;
      int is_scan;
      int format_num;
      int first_arg_num;
...
record_format_info (function_ident, is_scan, format_num, first_arg_num)
      ...
      int first_arg_num;
{
	char *name = IDENTIFIER_POINTER(function_ident);
	char *n = (char *)xmalloc(strlen(name) + 1);
	strcpy(n, name);
	_record_format_info (n, is_scan, format_num, first_arg_num);
}
...
{
  _record_format_info ("printf", 0, 1, 2);
  _record_format_info ("fprintf", 0, 2, 3);
  _record_format_info ("sprintf", 0, 2, 3);
  _record_format_info ("scanf", 1, 1, 2);
  _record_format_info ("fscanf", 1, 2, 3);
  _record_format_info ("sscanf", 1, 2, 3);
  _record_format_info ("vprintf", 0, 1, 0);
  _record_format_info ("vfprintf", 0, 2, 0);
  _record_format_info ("vsprintf", 0, 2, 0);
}
     ...
     int require_complete;
{
  ...
  if (TREE_CODE (function) == FUNCTION_DECL)
    {
	  ...
	  if (name)
		AKPMname = IDENTIFIER_POINTER(name);
#endif
  ...
  if (warn_format && AKPMname != 0)
    {
      unsigned int i;

      /* See if this function is a format function.  */
      for (i = 0; i < function_info_entries; i++)
	if (!strcmp(function_info_table[i].function_ident, AKPMname))
	  {
     ...
     tree arg1, arg2;
{
  ...
  if (code == MEMBER_REF)
...
#ifdef TMS34010
	{
		...
		op0 = default_conversion (arg1);
		op1 = default_conversion (arg2);
		type0 = TREE_TYPE (op0);
		type1 = TREE_TYPE (op1);
		...
		if (code0 != ERROR_MARK && code1 != ERROR_MARK)
		{
			if (code == PLUS_EXPR || code == MINUS_EXPR)
			{
				if (code0 == POINTER_TYPE && code1 == INTEGER_TYPE)
					arg2 = build_binary_op(MULT_EXPR, arg2, size_int(GSP8), 1);
				if (code0 == INTEGER_TYPE && code1 == POINTER_TYPE)
					arg1 = build_binary_op(MULT_EXPR, arg1, size_int(GSP8), 1);
			}
		}
	}
#endif
...
#ifdef TMS34010
	op1 = size_binop(MULT_EXPR, op1, size_int(GSP8));
#endif
...
#ifdef TMS34010
		offset = size_binop(MULT_EXPR, offset, size_int(GSP8));
#endif
...
#ifdef TMS34010
		inc = build_binary_op(MULT_EXPR, inc, size_int(GSP8), 0);
#endif

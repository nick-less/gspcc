     ...
     int len, align;
{
  ...
  if (!(data.autinc_from && data.autinc_to)
      && move_by_pieces_ninsns (len / GSP8, align) > 2)
    {
...
move_by_pieces_1 (genfun, mode, data)
     ...
     struct move_by_pieces *data;
{
  register int size = GET_MODE_SIZE (mode) * GSP8;
     ...
     int extra, below;
{
  register rtx temp;
  if (CONSTANT_P (size))
    anti_adjust_stack (plus_constant (size, extra), GSP8);
  else if (GET_CODE (size) == REG && extra == 0)
    anti_adjust_stack (size, GSP8);
  else
    {
     ...
     rtx args_so_far;
{
  ...
  if (mode == BLKmode)
	...
	xinner = change_address (xinner, BLKmode,
				 plus_constant (XEXP (xinner, 0), used * GSP8));
	  ...
	  && PUSH_ROUNDING (INTVAL (size)) == INTVAL (size))
	{
	  ...
	  if (extra && args_addr == 0
	    ...
	    anti_adjust_stack (GEN_INT (extra), GSP8);
  ...
  else if (partial > 0)
    {
	 ...
	 that we must make space for but need not store.  */
#ifndef AKPM
#if (PARM_BOUNDARY / BITS_PER_WORD) == 0
		int offset = 0;
#else
      ...
      int skip;
#ifndef AKPM
	warning("Offset problem in %s, %d", __FILE__, __LINE__);
#endif
      /* Push padding now if padding above and stack grows down,
	 or if padding below and stack grows up.
	 But if space already allocated, this has already been done.  */
...
get_inner_reference (exp, pbitsize, pbitpos, poffset, pmode, punsignedp, pvolatilep)
     ...
     int *pvolatilep;
{
	tree ret = _get_inner_reference (exp, pbitsize, pbitpos,
							poffset, pmode, punsignedp, pvolatilep);
	if (!flags('I'))
	{
		*pbitpos = (*pbitpos & ~7) * GSP8 + (*pbitpos & 7);
		if (*poffset)
			*poffset = size_binop(MULT_EXPR, *poffset, size_int(8));
	}
	return ret;
}

tree
_get_inner_reference (exp, pbitsize, pbitpos, poffset, pmode, punsignedp, pvolatilep)
  ...
  while (1)
      ...
      else if (TREE_CODE (exp) == ARRAY_REF)
	{
	  ...
	  if (TREE_CODE (index) == INTEGER_CST
	    ...
	    {
	    	*pbitpos += TREE_INT_CST_LOW (index) / GSP8;
#ifdef TMS34010
			warning("did /8");
#endif
	    }
  ...
  switch (DECL_FUNCTION_CODE (fndecl))
      ...
      else
	{
	  ...
	  tem = memory_address (Pmode,
				plus_constant (tem, GET_MODE_SIZE (Pmode) * GSP8));
...
do_pending_stack_adjust ()
{
  if (inhibit_defer_pop == 0)
	...
	adjust_stack (GEN_INT (pending_stack_adjust), GSP1);
      pending_stack_adjust = 0;
    }
}
...
#ifdef TMS34010
  if (flag_force_mem && mode != QImode && mode != HImode)
    {
     ...
     enum machine_mode mode;
{
  ...
  index = gen_rtx (PLUS, Pmode,
		   gen_rtx (MULT, Pmode, index,
			    GEN_INT (GET_MODE_SIZE (CASE_VECTOR_MODE) * GSP8)),
